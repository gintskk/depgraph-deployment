...
(
    -- leave the key-contents to the plugin implementation
...
CREATE UNIQUE INDEX CONCURRENTLY unique_vuln_x_pkg_ver ON vulnerabilities_x_package_versions USING btree (vulnerability_id, package_version_id);
ALTER TABLE vulnerabilities_x_package_versions ADD CONSTRAINT unique_vuln_x_pkg_ver UNIQUE USING INDEX unique_vuln_x_pkg_ver;

CREATE UNIQUE INDEX CONCURRENTLY unique_vuln_x_callable ON vulnerabilities_x_callables USING btree (vulnerability_id, callable_id);
ALTER TABLE vulnerabilities_x_callables ADD CONSTRAINT unique_vuln_x_callable UNIQUE USING INDEX unique_vuln_x_callable;
